...
const setUserAvatar=async (username,avatar)=>{
    try {
        ...
        if (avatarUserList.map(user => user.name).includes(username)){
            await usequery(`update useravatar set avatar='${avatar}' where name = '${username}'`)
            return "设置成功"
        }else {
